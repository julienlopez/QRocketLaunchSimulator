...
{
	Q_ASSERT(m_engine);
	m_altitude_lcd->display(m_engine->get().altitude().value());
	m_speed_lcd->display(m_engine->get().state().velocity.norm().value());
	m_acceleration_lcd->display(m_engine->get().currentAcceleration().norm().value());
	m_time_lcd->display(m_engine->get().currentTime().value());
}
